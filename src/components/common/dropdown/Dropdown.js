...
class Dropdown extends PureComponent {
  ...
  renderBase(props) {
    ...
    return (
      <View style={styles.textSelectedContainer}>
        <Text style={{ fontSize: 14, paddingRight: 5 }}>{label}</Text>
        <Text style={styles.textSelectedContent}>{value}</Text>
      </View>
    );
  }
  ...
  renderItem({ item, index }) {
    ...
    const itemcolor = index === selected ? selectedItemColor : itemColor;
    const selectedColor = ~selected ? itemcolor : selectedItemColor;
    const color = disabled ? disabledItemColor : selectedColor;
